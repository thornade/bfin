...
def get_page_stock2(ref,type="isin",last=-1,start="2021-01-01",PC_bearish=0.4,PC_bullish=19,RSI_bearish=60,RSI_bullish=30,period_ana=90):
    if type == "isin":
        (name,c,df)=graph_isin_with_bourso(ref,last=last,start=start)
    
    #df_bm=calculate_best_moves(df,PC_bearish=PC_bearish,PC_bullish=PC_bullish,RSI_bearish=RSI_bearish,RSI_bullish=RSI_bullish,duration_limit=45)
    # Add low limit to PC & RSI
    df_bm=calcbm_HC_hit_HL(df,PC_bearish=PC_bearish,PC_bullish=PC_bullish,RSI_bearish=RSI_bearish,RSI_bullish=RSI_bullish,duration_limit=45)
    df_bm_ana=analyse_best_moves(df=df,dfbm=df_bm,period=period_ana)
    if len(df_bm_ana) > 0:
        df_bm_ana_display=df_bm_ana[["date_buy","date_sell","best_date_sell","days","best_days","PC_buy","PC_sell","best_PC_sell","diffp","best_diffp","RSI_move","RSI","d-diffp"]]
        df_bm_ana_display["date_sell"]=df_bm_ana_display["date_sell"].apply(lambda x:str(x).split(" ")[0])
...
def calcbm_HC_hit_HL(df,PC_bullish=19,PC_bearish=0.1,RSI_bullish=30,RSI_bearish=70,duration
    if ("PC_high-close%" in df.columns) & ("RSI" in df.columns):
        ...
        ldiff=[]
        for uc in df[(df["PC_high-close%"]>=PC_bullish) & (df["RSI"]<=RSI_bullish) & (df["diff_PCHC_HL"] < 1)].reset_index()["index"].to_list():
            _d=df[((df.index == uc)) | ((df.index > uc) & (df["PC_high-close%"] <= PC_bearish) & (df["RSI"]>=RSI_bearish)) | ((df.index > uc + duration_limit)) ].head(2)
            _e=_d.reset_index()
            diff=_e.iloc[-1]["Close"]-_e.iloc[0]["Close"]
            diffp=(_e.iloc[-1]["Close"]-_e.iloc[0]["Close"]) / _e.iloc[0]["Close"] * 100
...
def viewit2(isin,start="2021-01-01"):
    RSI_bearish=75
    RSI_bullish=30
    PC_bearish=0
    PC_bullish=20

    (n,p,df,dfbm,dfbm_ana)=get_page_stock2(ref=isin,type="isin",start=start,PC_bearish=PC_bearish,PC_bullish=PC_bullish,RSI_bearish=RSI_bearish,RSI_bullish=RSI_bullish,period_ana=60)
    pPCHC_Bearish=int(df["PC_high-close%"].quantile(0.03))
    pPCHC_Bullish=int(df["PC_high-close%"].quantile(0.87))
    pRSI_Bullish=            int(df["RSI"].quantile(0.30))
    pRSI_Bearish=            int(df["RSI"].quantile(0.80))
    (n,p,df,dfbm,dfbm_ana)=get_page_stock2(ref=isin,type="isin",start=start,PC_bearish=pPCHC_Bearish,PC_bullish=pPCHC_Bullish,RSI_bearish=pRSI_Bearish,RSI_bullish=pRSI_Bullish,period_ana=60)
